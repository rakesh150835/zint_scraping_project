...
def get_distance_from_mapbox(origin, destination):
    try:
...
def cal_distance(postcode_1, postcode_2):
    try:  # Replace with your Mapbox token
        ...
        coords_1 = get_lat_long_from_postcode(postcode_1)
        coords_2 = get_lat_long_from_postcode(postcode_2)

        # calculate distance between two postcodes
        if coords_1 and coords_2:
            distance = get_distance_from_mapbox(coords_1, coords_2)
            if distance:
                return distance
            else:       
                return None
